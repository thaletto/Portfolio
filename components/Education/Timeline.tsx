...
export default function Timeline({ institution, course, cgpa, location, start, end }: TimelineProps) {
    return (
        <div className="mb-8 flex items-start relative">
            ...
            {/* Content section */}
            <div className="ml-8">
                <p className={acornLight.className + " flex items-center text-lg md:text-2xl text-balance text-customGreen mb-1"}>
                    <FaUniversity className="mr-2 md:text-lg mb-1"/> <strong>{institution}</strong>
                </p>
                <p className="text-sm md:text-lg text-slate-300 text-balance">{course} | {cgpa}</p>
                <p className="text-sm md:text-lg text-slate-300 flex items-center text-balance">
